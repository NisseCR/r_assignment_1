...
Might be due to sensor threshold for measuring windspeed. In any case, these values fall within the distribution, it is only odd that there's a small increment between the `0.00` occurrences and the values thereafter (e.g. `0.0869`).
These might also be possible missing values which are replaced with `0.00`.  
In any case, we cannot know for certain. Since the number of occurrences for `0.00` is in line with neighboring values, we choose to keep them.
