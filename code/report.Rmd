...
Since there was an indication of outliers, we decided to detect them using quantiles. We made the following boxplots:
...
The boxplots further support our observations from earlier. Temperature and feeling temperature are distributed very normally and display no outliers on the boxplots. Humidity has a a few "0" values that are flagged as outliers. In the wind speed, the large number of "0" values skews the distibution towards the smaller values, leaving a lot of the high wind speeds as outliers. The boxplot of number of bike rentals reveals a large number of outliers in our outcome variable. As we already saw that the night hours are dominated by small rental number, thus skewing the distribution to the left and leaving the high rental values as outliers. 
...
counting the segments of the day in hours for the `morning-noon` and
...
-   Call: Shows the function call to compute our regression model, which
    ...
    $$
-   Residuals: A quick view of the distribution of the residuals, which
    by definition have a mean zero.
...
regression model, this means that for every Celsius degree increase,
*average* rental bike is expected to **increase** by *2974.36*, after
controlling for humidity ("hum3"). Whereas, for each additional point of
humidity, *average* bike rental is expected to *decrease by 2082.68
units*, after controlling temperature.

## Significance testing - P values

Probability values (p-values) measures the strengthen evidence of the
null hypothesis with a fraction or decimal value between 0 and 1. **The
closer the value is toward 0, the stronger the evidence against the null
hypothesis.**

Viewing this in our regression model, the predictor variables results
obtained that *p \<2.2e-16*. Thus, **rejecting our null hypothesis**. We
conclude that there is a statistical significance or **strong**
relationship of temperature and humidity with bike rental, due to our
smaller p-values.
...
segments of hours in the categories `morning-noon` and `eve`(evening).
...
having an **average positive estimation** of *587.58 units*, meanwhile,
humidity showcases a **negative average estimation** of *94.16 units*.

**Intercept and reference groups**

-   `hr_seg` represents a segment of the day in hours and considered the
    reference group in this example.
-   Intercept represent the average bike rental for a bike rented within
    a segment of the day (`hr_seg`), assuming all resting variables are
    held constant.
...
-   The **average** estimated value for renting a bike during the
    **morning-noon segment** of the day is of *961.65* units, **higher**
    in comparison to the **evening segment** of the day. In the evening
    segment of the day, the **average** estimated value for renting a
    bike is *548.62 units*.

-   After controlling for temperature and humidity, the *average*
    estimated bike rental during the **morning-noon segment** is equal
    to *1627.07 units*, whereas the *average* estimated bike rental
    during the **evening** is *3076.57* units. Around **1449.5 bike
    rental units increased** during the evening compared to the
    morning-noon segment, after controlling for temperature and
    humidity.
...
for their average estimated values. **As temperature increases, humidity
decreases**. Thus, cross validating our results by being consistent.
...
Concerning the **second model** including `hr_seg` as the *predictor
variable*, **humidity does not necessarily have a strong statistical
relationship with bike rental**, where *p = 0.372*. Certainly, higher
than its p-value in the regression model.
