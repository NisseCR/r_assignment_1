...
Some of our categorical variables (i.e. `workingday`, `weathersit`, `hr`) are interpreted as integers, rather than factors. We transform these data to factors for easier processing and modelling.
...
By combining the component outcome variables mentioned in [Variable description](#vardesc), we create `registered_prop`, which is the proportion subscribed rentals compared to the total rental count. Consequently, we use the mean of this variable to create a binary outcome variable called `more_registered`. This outcome is a 'success' when the proportion of registered users is larger than its respective average.
...
Since the summaries are not giving a precise enough picture, we make histograms of the variables.
...
Lastly, the `weathersit` category `4` seems to be barely present according to the [distributions](#dists).

We investigate the "problematic" variables: humidity, wind speed and number of rentals and weathersit further below:

### Rental count outliers

```{r hour influence, warning = FALSE, message = FALSE}
data %>%
  group_by(hr, more_registered) %>%
  summarise(
    cnt = sum(cnt)
  ) %>%
  ggplot(aes(hr, cnt, fill = more_registered)) +
...
While the `0.00` values fall within the distribution, it is only odd that there's a small increment between the `0.00` occurrences and the values thereafter (e.g. `0.0869`). This might be due to sensor threshold for measuring wind speed, or these might also be possible missing values which were replaced with `0.00`. Since we cannot know for certain and the number of occurrences for `0.00` is in line with neigbouring values, we choose to keep them.
...
We first determine the amount of occurrences within our data *per* `weathersit` category:
...
We note that weathersit `4` only has 3 observations within a sample size of 17k+ records. Due to this, we decide this is a categorical outlier and will hence be removed by the data to prevent contamination of our model:
...
The feeling temperature, represented as `atemp` within the data, by definition might be a combination of other predictor variables such as `temp`, `windspeed` and `hum`. To check whether independent variables might correlate (multicollinearity), we scatter plot `atemp` in particular with other predictor variables to observe potential covariance.
...
Plotting the amount of success and failure occurrences per `workingday` category gives us insight into how the variable possibly affects our outcome. From the bar graph, we can conclude that the proportion of registered users compared to the total amount of users changes dramatically between non-working days and workdays. In particular, the proportion of registered users sits below its respective average during non-working days, whilst the majority of observations during workdays have a registered proportion higher than its average.
...
By inspecting the average proportion values per `workingday` category, we can indeed conclude that the proportion is more likely to exceed its respective mean threshold when `workingday` is set to `1`.

```{r change workingday vs. outcome}
...
Thus, `workingday` can possibly have a strong effect on our outcome variable.
...
```{r check deviance residuals, warning = FALSE}
...
Based on the model comparison earlier, we chose model 6, which is represented by the following formula:$$
    logit(\hat{\pi}) = \hat{\beta_0} +  \hat{\beta_1}workingday + \hat{\beta_2} windspeed + \hat{\beta_3} temp + \hat{\beta_4} hum + \hat{\beta_5} workingday * windspeed + $$ $$ +\hat{\beta_6} whethersit2 + \hat{\beta_7}whethersit3 + \hat{\beta_8} workingday * temp + \hat{\beta_9} workingday * hum
    $$ The model includes workingday as a categorical moderator of windspeed, temperature and humidity. Additionally, class 1, which represents the lightest wheather phenonena is the reference category for weathersit and a weekend day is the reference for workingday. Lastly, in this final model the means of the predictors are centered.

Below we present the model summary:
...
Model quality for logistic regression can be assessed based on how well the model classifies cases as successes and failures. For classification testing of our model we used a confusion matrix. To produce one, the model has to classify each case based on whether the probability is a success. We decided to use a threshold of 0.5, as in: if the probability of the proportion of registered users is higher than average is 50% then the case is classified as a "success". We decided to use the standard 0.5 threshold because we did not find any substantial reason to justifying changing it to a different value.
...
Based on the confusion matrix we find the following characteristics of our model:
...
In the table we can see that the final model is fairly accurate (with accuracy at 73.7%). It is not particularly high, but considering our application people's behavior based on weather (both notoriously hard to predict) we deem it sufficient. Comparing sensitivity (60.1%) and specificity (84.0%) reveals that the model is better at identifying cases with below average registered rentals ratio than those above average. The same is revealed by the PPV (73.5%) and NPV (77.3%), though here the diffrence is smaller.

Further characteristics of the model can be seen below:

```{r further characteristics}
cMat$overall
cMat$byClass
```
...
```{r, message=FALSE}
rocData <- data %>% 
  pROC::roc(more_registered, piHat)
plot(rocData)
```

The ROC curve depicts the idea is that Sensitivity and Specificity are always a trade off. The more you reject any data point as not a success the more likely you are to correctly reject it, however the likelihood of correctly classifying it as a success decreases. The shape of this curve suggests that our model is fairly balanced between the two, even though our Specificity is higher to Sensitivity. Generally, the closer the model is to the $(1,1)$ corner the better the model is at classifing the data.

## Significance of predictors

```{r coefficients_final_model}
summary(model6)$coefficients

```

As can be seen above all of the predictor slopes, interaction terms and the intercept are significantly different from 0.

## Interpretation coefficients & intercept {.tabset}

To interpret the intercept and coefficients in more understandable units we convert them with an exponential function:

```{r exp_coefficients}
# we transform the coefficients with the exponential function, so they can be interpreted as the odds ratio (not log of it)
coef(model6) %>% exp()
```

It is important to note that the variables windspeed, temperature and humidity were normalized, thus a unit step in them is not expressed in any specific units (such as km/h) and a unit step increase represents a really big change in the variable value. Additionally, the continuous variables have been mean-centered, which changes the interpretation of the intercept and of partial effects. Lastly, the reference categories for weathersit and workingday are weathersit = 1 (lightest weather phenomena) and a non-working day accordingly.

### Intercept

The intercept equals 0.302. Thus, the odds ratio of the proportion of registered rental bikes being higher than average to lower than average is 0.302 times lower when weathersit is equal to 1 (lightest weather phenomena), it is a non-workingday and humidity, temperature and windspeed are at their mean value.

### Workingday

The slope of workingday1 is equal to 8.356. Thus on a workingday the odds ratio of the proportion of registered rental bikes being higher than average to lower than average increases 8.356 times the odds ratio on a non-working day; if windspeed, temperature and humidity are fixed and 'wheathersit' is equal to 1. Alternatively, it can be understood as for a workingday the odds ratio increases by 735.6% compared to the non-working day.

### Windspeed

The slope of windspeed is equal to 3.277. This means that for a unit increase in windspeed the odds ratio of the proportion of registered rental bikes being higher than average to lower than average increases 3.277 times, if temperature and humidity are fixed and 'workingday' and 'wheathersit' are equal to their reference categories (non-working day and 1, accordingly).

### Temperature

The temperature's coefficient is equal to $3.26*10^{-4}$. This means that for a unit increase in temperature the odds ratio of the proportion of registered rental bikes being higher than average to lower than average decreses $3.26*10^{-4}$ times, if windspeed and humidity are fixed and 'workingday' and 'wheathersit' are equal to their reference categories.

### Humidity

The humidity's coefficient is equal to 2.465. This means that for a unit increase in humidity the odds ratio increases 2.465 times; if windspeed and temperature are fixed and 'workingday' and 'wheathersit' are equal to their reference categories.

### Weathersit

'Weathersit' represents the categorical variable that indicates the severity of weather phenomena. As a reminder, the final categories of `weathersit` represent the following weather phenomena and their combinations:

1.  Clear, Few clouds, Partly cloudy, Partly cloudy
2.  Mist and Cloudy, Mist and Broken clouds, Mist and Few clouds, Mist
3.  Light Snow, Light Rain and Thunderstorm and Scattered clouds, Light Rain and Scattered clouds

The coefficient of weathersit2 represents the fact that the odds ratio for weathersit equal to 2 is (`r round(8.706174e-01, digits = 3)`) times lower than the odds ratio for weathersit equal to 1. Similarly, weathersit3's coefficient represents that on average the odds ratio increase (`r round(1.258661e+00, digits = 3)`) compared to the reference category (1), after controlling for all variables.

### Interaction terms

The interaction terms $\hat{\beta_7}$, $\hat{\beta_8}$ and $\hat{\beta_9}$ represent the moderation of weathersit on the effect of respectively windspeed, temperature and humidity on the odds ratio. Since they are significant, the effect of these continuous variables is different on a working day as opposed to the ones we analyzed above on a non-working day.

```{r calculating_int_coeff, echo=FALSE}
coeffwind <- 3.276739e+00 * 3.976885e-01
coefftemp <-  3.263983e-04 * 3.490583e+01
coeffhum <- 1.175950e+01 * 4.024118e-01
```

Thus, on a workingday a unit increase in windspeed leads to a `r round(coeffwind, digits = 3)` ($e^{\hat{\beta_2} + \hat{\beta_7}}$) change in the odds ratio, when controlling for humidity, temperature and weathersit equal to 1. For a unit increase in temperature on a workingday, the average odds ratio increases by `r round(coefftemp, digits = 3)` ($e^{\hat{\beta_3} + \hat{\beta_8}}$), when controlling for humidity, windspeed and weathersit equal to 1. And lastly, for a unit increase in humidity on a workingday, the average odds ratio increases by `r round(coeffhum, digits = 3)` ($e^{\hat{\beta_4} + \hat{\beta_9}}$), when controlling for temperature, windspeed and weathersit equal to 1.

## Answering the research question

The research question is: **To what extent can weather data predict the likelihood of the ratio of registered rentals to casual rentals being above average, and is this effect different for working days than for non-working days?**

For a working day the odds ratio increases by 735.6% compared to the non-working day. This is an outcome that we expected, as we showed in the EDA that there are more casual users on non-working days. Additionally, in real life people need to travel to work on working days, if they chose to do so by the rental biking system, it is reasonable for them to register, since their use is regular, and use it more often on working days.

A unit increase in windspeed increases the odds by 227.7% (controlling for other variables). Thus, the odds of there being relatively more registered users than casual increases with stronger winds. A similar relationship can be seen with humidity (146.5% increase) or temperature ($3.26*10^{-2}%$) decrease, here really high temperatures are the harsh conditions). A possible explanation would be that the worst conditions deter casual users. We expect the registered users to be more frequent users of the system, possibly for reasons such as commuting to work. Our hypothesis was that they will be more likely to use the system in worse conditions, as they already paid for it (assuming registration needs payment, alternatively they already committed to the effort to register) and they do need to get somewhere on regular basis, thus biking would be their habit.

Weathersit has somewhat surprising results. Going from weathersit1 to weathersit2, there is a decrease in odds ratio of 12.9%, i.e. there should be less registered rentals to casual rentals. However, going from weathersit1 to weathersit3 the odds ratio increases by 25.9%, which suggests a bigger number of registered rentals to casual ones. We suspect there might be another variable that explains this effect or possibly weathersit effects one of the variables that compose it in a more understandable manner. Alternatively, this is just the relationship for some reason that we did not come up with and could be found with further investigation.

The effect of both windspeed and temperature seem to go down when using workingday as a moderator. However, the effect of humidity increases when using workingday as a moderator.
