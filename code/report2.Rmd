...
## Interpretation of coefficents and the intercept
...
The coefficients of weathersit2 (-0.139) and weathersit3 (0.230) represent the average diffrence in odds ratio between the reference  category (1) and categories 2 and 3 respectively, after controling for windspeed, temperature, humidity and workingday. 
...
For unit increase in temperature on a workingday the average odds ratio increases by `r round(coefftemp, digits = 3)` ($e^{\hat{\beta_3} + \hat{\beta_8}}$)

And lastly, for unit increase in humidity on a workingday the average odds ratio increases by `r round(coeffhum, digits = 3)` ($e^{\hat{\beta_4} + \hat{\beta_9}}$)
