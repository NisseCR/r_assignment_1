...
It is important to note that the variables windspeed, temperature and humidity were standardized, thus a unit step in them is not expressed in any specific units (such as km/h) and a unit step increase represents a really big change in the variable value. Additionally, the continuous variables have been standardized, which changes the interpretation of the intercept and of partial effects. Lastly, the reference categories for wheathersit and workingday are weathersit = 1 (lightest weather phenomena) and a non-working day accordingly. 
...
### Temperature
The temperature's coefficient is equal to `r round(3.263983e-04, digits = 6)`. This means that for a unit increase in temperature the odds ratio of the proportion of registered rental bikes being higher than average to lower than average decreses `r round(3.263983e-04, digits = 6)` times, if windspeed and humidity are fixed and 'workingday' and 'wheathersit' are equal to their reference categories. 
