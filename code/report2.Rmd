...
create_outcome <- function(prop, mean) {
...
data <- data %>%
  mutate(
    ...
    more_registered = create_outcome(registered_prop, mean(registered_prop))
  )
...
There need to be more observations, then there are predictors. That assumption is easily met; there are 17376 observations, but we are only making use of five predictors.

Furthermore, the model should not suffer from multicollinearity. We appeal to the variance inflation factor, in order to check whether we are dealing with multicollinearity.

```{r}
vif(model6, type = 'predictor')
```

Originally, the VIF in model6 used to be too high to deal with (>10). After all, it makes sense that the interaction terms highly correlate with their respective variables. However, since we applied mean centering, the values are sufficient to continue with.
...
Of course, our outcome variable is binary.
The balance between both classes looks quite nice. There are a little more data points in the second class, but nothing too egregious.
...
A logistic regression assumes a linear relationship between the logit of the outcome, and the continuous predictor variables. 
...
All plots show a linear relationship.

## Influential data points

The model should not be biased due to extremely influential data points. We can measure Cook's distance, as well as plot a residuals vs leverage plot, to check whether this assumption is violated or not.

```{r}
plot(model6, which = c(4, 5))
```

No points seem to fall outside of the thresholds for a too large Cook's distance, or influential leverage. 
