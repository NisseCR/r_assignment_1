...
Then, we check if this effect is moderated by whether it is a working day, or a non-working day (holiday or weekend). After all, a registered user might feel more inclined to make use of their subscription when going to work, even when the weather is bad, whereas a non-registered user might instead opt to use public transport, or other forms of transport.
...
### Weathersit outliers
We first determing the amount of occurrences within our data *per* `weathersit` category:
...
We note that weathersit `4` only has 3 observations within a sample size of 17k+ records. Due to this, we ascertain this is a categorical outlier and will hence be removed by the data to prevent contamination of our model:
...
This is an unexpected outcome, as it means that a change from light weather (weathersit1) to a somewhat more severe weather conditions (weathersit2) leads to a decrease in odds ratio of the registered rentals ratio, ie. there should be less registered rentals to casual rentals. At the same time the change from weathersit1 to severe weather conditions (weathersit3) leads to an increase in the odds ratio, thus suggests a bigger number of registered rentals to casual ones. We suspect there might be another variable that explains this effect or possibly weathersit effects one of the variables that compose it in a more understandable manner. Alternatively, this is just the relationship for some reason that we did not come up with and could be found with further investigation. 


### Interaction terms

The interaction terms $\hat{\beta_7}$, $\hat{\beta_8}$ and $\hat{\beta_9}$ represent the moderation of weathersit on the effect of respectively windspeed, temperature and humidity on the odds ratio. Since they are significant that means that the effect of these continuous variables is different on a working day as opposed to the ones we analyzed above on a non-working day. 
